...
not runtime. Native OMP is the current harness: `.omp/config.yml` is empty,
`.omp/AGENTS.md` imports the root guide and `PROJECT.md`, and
`.omp/APPEND_SYSTEM.md` contains only planner/Main allocation. `PROJECT.md`
selects Development Sprint by default. The next-major I5 qualification is
paused and deactivated, remains incomplete, and needs a separately selected
qualification outcome before any acceptance procedure can resume. Adoption and
next-major packets and companions remain preserved historical evidence; their
procedures are not current authority. Current contract sections describe
implemented behavior, not qualification or delivery approval. Product
examples/workflows remain in [README.md](README.md) and packaged
[guidance](src/xray/guidance.md); this file owns the complete contract.
...
| Development harness | `AGENTS.md`, `PROJECT.md`, this file, `CONTINUE.md`, `TEMPLATE_MANIFEST.md`, `docs/`, `.omp/`, `.agents/`, `Makefile` | Native OMP policy, Main allocation, recovery, adoption, and gates; product source does not import it. |
...
not change shared engine semantics. Main controls native OMP policy and may
strengthen harness checks but may not redefine product APIs or schemas.
...
none activates or accepts the paused I5 milestone.

The native OMP layer is intentionally declarative: `.omp/config.yml` is `{}`,
`.omp/AGENTS.md` imports `AGENTS.md` and `PROJECT.md`, and
`.omp/APPEND_SYSTEM.md` defines only planner/Main allocation. `uv run python
.omp/validate.py` checks the empty project layer, absent legacy harness,
required bundled role inventory, and no custom `.omp/agents`; `uv run python
.omp/validate_project_readiness.py` retains factual readiness checks without
prose-size quotas. The Makefile composes available focused/static/full/package/
MCP/CLI, harness, readiness, and cleanliness gates; it does not make every gate
a routine requirement. Transformation evidence and packet digests preserve
historical adoption/qualification claims and are not regenerated by routine
edits.

## Current change policy

Main coordinates only the affected paths and integrates them serially; related
paths are not a license to widen scope or create paper synchronization work.
...
- Native OMP policy, planner/Main allocation, recovery, or gate changes update
  only the affected `.omp/` files, `AGENTS.md`, `PROJECT.md`, `CONTINUE.md`,
  validators, standards, applicability links, and current evidence. Frozen
  packets, companions, and byte-invariant examples remain unchanged unless a
  separately selected qualification outcome authorizes a material boundary
  change.
